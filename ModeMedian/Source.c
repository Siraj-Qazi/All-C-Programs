...
void median(unsigned int answer[])
{
   ...
   unsigned int median = answer[SIZE / 2];
		  ...
		  for (rating = 1; rating <= 9; ++rating) {
			 ...
			 for (h = 1; h <= freq[rating]; ++h) {
			  ...
			  puts(""); // being new line of output
			    ...
			    if (modeArr[1]==largest)
			   {
				   printf("\nThe mode is the most frequent value.\n"
					   "For this run the mode value(s) are %u %u which occurred"
					   " %u times.\n", modeArr[0],modeArr[1], largest);
			   }
			   else 
			   {
				   printf("\nThe mode is the most frequent value.\n"
					   "For this run the mode value is %u which occurred"
					   " %u times.\n", modeArr[0], largest);
			   }
